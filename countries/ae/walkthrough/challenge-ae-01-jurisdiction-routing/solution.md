...
> Walkthrough for `Sovereignty Summit United Arab Emirates 2026` / Challenge AE-01.
...
5. Add the **sector overlay** (`cbuae-bank`, `dha-health`, `tdra-telecom`, or
   `none`).
6. Confirm data stays in `uaenorth` or
   `uaecentral`. If it crosses legal perimeters, open an
...
| Policy | Effect | Why |
|---|---|---|
| Allowed locations | Deny | Keep workloads in `uaenorth` or `uaecentral` |
