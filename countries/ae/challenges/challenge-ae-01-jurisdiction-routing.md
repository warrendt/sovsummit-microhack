...
You are the platform lead for **A government-related citizen-services platform integrating with onshore entities, a DIFC bank and an ADGM analytics provider**.
...
`uaenorth`. The Azure region is the same, but the legal
perimeter is not.
...
6. **Where does the data land?**
   - Approved hosting locations are `uaenorth` and
     `uaecentral` only.
...
- If the dataset never leaves `uaenorth` but is shared
...
- **Free-zone status follows the legal establishment, not the subnet.** Running a
  DIFC workload in `uaenorth` does not convert it into an
...
|---|---|---|---|---|---|---|---|
| Case-management API | Onshore | Federal PDPL | None / TDRA | `mg-ae-federal` | `uaenorth`, `uaecentral` | No | N/A |
| Refunds / treasury app | DIFC | DIFC DP Law | Financial-sector review as applicable | `mg-ae-difc` | `uaenorth`, `uaecentral` | Yes | Required |
| Analytics workspace | ADGM | ADGM DPR 2021 | None | `mg-ae-adgm` | `uaenorth`, `uaecentral` | Yes | Required |
