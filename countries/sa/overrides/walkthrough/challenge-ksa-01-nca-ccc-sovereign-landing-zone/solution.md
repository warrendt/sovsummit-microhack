...
az monitor log-analytics workspace create   -g $PLATFORM_RG   -n $LAW_NAME   -l ${country.azure.primary_region}

az keyvault create   --name $KV_NAME   --resource-group $PLATFORM_RG   --location ${country.azure.primary_region}   --sku ${country.azure.cmk_hsm_sku}   --enable-rbac-authorization true   --enable-purge-protection true   --retention-days 90
...
1. **Normal workload scope** (`mg-sovsummit-ksa` or `mg-ksa-workloads`) that allows
   only `${country.azure.primary_region}`.
2. **DR exception scope** (`mg-ksa-dr-exception`) that additionally allows
   `${country.azure.paired_region}`, but only with explicit approval metadata.

### Custom policy: `${country.azure.paired_region}` requires approval tag
...
```json
{
  ...
  "parameters": {
    "drRegion": {
      "type": "String",
      "defaultValue": "${country.azure.paired_region}"
    }
  },
  "policyRule": {
    "if": {
...
| `${country.azure.paired_region}` requires `ksa-dr-approved=true` | DR exception | Deny |
...
az group create -n rg-dr-test -l ${country.azure.paired_region}
...
| HSM-backed CMK | Key custody stays in `${country.azure.primary_region}` with auditable separation of duties. |
