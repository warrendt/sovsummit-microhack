...
> Walkthrough for `Sovereignty Summit Nigeria 2026` / Challenge NG-03.
...
Use a policy initiative that allows only `southafricanorth` and
`southafricawest` for this landing zone.
...
az policy assignment create \
  ...
  --params '{"listOfAllowedLocations":{"value":["southafricanorth","southafricawest"]}}'
```
...
Provision the vault in `southafricanorth`:
...
```text
Digital channel app in southafricanorth
  ...
  -> App stores tokenised record in SQL / Storage in southafricanorth
```

Minimum design rules:

- raw BVN / NIN / PAN never persist in `southafricanorth`;
...
| `tokenised-operational` | `southafricanorth` | CMK + Private Link + tags |
| `derived-analytics` | `southafricanorth` / `southafricawest` | CMK + policy + DR |
...
1. **Destination:** `southafricanorth` / `southafricawest`.
...
6. **Breach workflow:** show how the NDPC is notified within
   `72` hours and how customers are
...
only the surrogate token reaches `southafricanorth`.
