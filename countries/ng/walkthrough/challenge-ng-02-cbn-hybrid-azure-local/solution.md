...
> Walkthrough for `Sovereignty Summit Nigeria 2026` / Challenge NG-02.
...
```powershell
. ./countries/ng/params/defaults.ps1
...
- `rg-ng-derived`   → `southafricanorth` /
...
| Allowed locations = `southafricanorth, southafricawest` | derived subscription | Deny |
...
Customer PII source
  ...
  -> Token-only stream
  -> Event Hubs in southafricanorth
...
- **Derived workloads:** Log Analytics workspace in `southafricanorth` -> Sentinel.
...
4. Exit and repatriation plan: how regulated workloads fall back to the Nigeria
   tier if `southafricanorth` is unavailable.
...
az vm create -n vm-ng-regulated-test -g rg-ng-derived \
  --image Ubuntu2204 --location southafricanorth \
...
Restore a tokenised backup into `southafricawest` and confirm the
