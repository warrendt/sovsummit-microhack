...
> **Qatar reality:** ${country.azure.confidential_compute_note}
...
You are **not** allowed to pretend confidential VMs exist in `${country.azure.primary_region}`.
...
- **HSM-backed CMK** in `${country.azure.primary_region}`,
...
|---|---|---|---|
| Hypervisor / host-admin visibility into workload memory | Hardware-backed encrypted memory + attestation | Not available in `${country.azure.primary_region}` | Residual memory-exposure risk remains |
...
- HSM-backed Key Vault in `${country.azure.primary_region}`,
...
- an exception / review note that this workload should be reassessed when Confidential Compute reaches GA in `${country.azure.primary_region}`.
...
- [ ] You explicitly state that Confidential Compute is **not** currently GA in `${country.azure.primary_region}`.
- [ ] Your solution uses CMK in an HSM-backed Key Vault in `${country.azure.primary_region}`.
...
- [ ] No raw PII, no raw financial data and no detokenisation secrets are replicated to `${country.azure.paired_region}`.
...
- ${country.regulatory.primary_law}
