...
> Walkthrough for `Sovereignty Summit Egypt 2026` / Challenge EG-01.
> Closest Azure region: `uaenorth`.
...
| Policy | Effect | Notes |
|---|---|---|
| Allowed locations | Deny | Allow only `uaenorth`, `uaecentral`. |
...
| Diagnostic settings to a Log Analytics workspace in `uaenorth` | DeployIfNotExists | Keeps audit evidence in the approved destination set. |
...
2. Why the workload is allowed in `uaenorth` instead of
...
az storage account create -n stuntagged -g rg-eg --location uaenorth --sku Standard_LRS
