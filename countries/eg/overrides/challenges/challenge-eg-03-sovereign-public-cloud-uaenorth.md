...
> **Primary region for this path:** `${country.azure.primary_region}` (${country.azure.primary_region_display})
> **Paired region for resilience:** `${country.azure.paired_region}` (${country.azure.paired_region_display})

## The situation

You are the platform architect for **${country.scenarios.public_sector_tenant}**.
...
- **"${country.azure.primary_region} is close enough, so everything can just go there."**

The approved position is more precise:

1. **For data that PDPL does not require to stay in Egypt**, the team may use
   `${country.azure.primary_region}` as the nearest Azure hyperscale region.
...
`${country.azure.primary_region}` and `${country.azure.paired_region}` with strong
platform controls, while keeping the in-country tokenisation boundary for the
restricted fields.

## Why `${country.azure.primary_region}`?

Use `${country.azure.primary_region}` for this pattern because it is:

- **the closest Azure hyperscale region to Egypt**, which reduces latency versus
  Europe-based alternatives;
- part of a **regional pair** with `${country.azure.paired_region}`, giving a clean
...
Build a **sovereign public-cloud landing zone** that proves `${country.azure.primary_region}`
...
- Defend when a workload belongs in `${country.azure.primary_region}` and when it
  belongs on Azure Local in Egypt.
- Pin resources to `${country.azure.primary_region}` / `${country.azure.paired_region}`
...
- Design a customer-controlled tokenisation pattern where restricted fields are
  substituted in Egypt before any dataset reaches `${country.azure.primary_region}`.
...
- Assign an initiative named **`Egypt Sovereign Public Cloud / UAE North`** that:
  - allows only `${country.azure.primary_region}` and `${country.azure.paired_region}`;
...
- Provision a **Premium Key Vault** in `${country.azure.primary_region}` with purge
  protection, soft delete, RBAC permission model, and an **RSA-HSM** key for CMK.
- Provision a **Storage account** in `${country.azure.primary_region}` that uses the
...
| Data class / field set | Default placement | Allowed in `${country.azure.primary_region}`? | Required safeguard |
|---|---|---|---|
| Public content, static web assets, non-personal operational metadata | `${country.azure.primary_region}` | Yes | Allowed-locations + Private Link where relevant |
| General personal data approved for transfer (for example support-case metadata, low-risk service telemetry without raw national ID) | `${country.azure.primary_region}` | Yes, with PDPC permit | CMK + Private Link + TIA + logging |
| Analytics datasets containing customer records after tokenisation / masking | `${country.azure.primary_region}` | Yes | Tokenise in Egypt first; no re-identification path in cloud |
...
- [ ] A deployment to `westeurope` is denied immediately by the allowed-locations
      initiative.
- [ ] `az keyvault show` confirms a **Premium** vault in `${country.azure.primary_region}`
...
- [ ] A sample dataset sent to `${country.azure.primary_region}` contains tokenised
      values only for the PDPL-restricted fields.
- [ ] The evidence pack clearly shows which data classes remain on Azure Local in
      Egypt and which are allowed into `${country.azure.primary_region}`.

## Guiding questions

- If `${country.azure.primary_region}` is the nearest region, why is an
...
- How would you explain to a regulator that using `${country.azure.primary_region}`
...
- Add a workbook that shows, per data class, whether the authoritative copy sits
  in Egypt or in `${country.azure.primary_region}`.
