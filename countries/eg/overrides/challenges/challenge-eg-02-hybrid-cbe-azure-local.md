...
- **Permitted external tier:** `${country.azure.primary_region}` for tokenised or
...
- Explain why **Azure Local** is the right answer for some Egyptian workloads
  even when `${country.azure.primary_region}` is available nearby.
...
- Keep the **token vault and re-identification service in Egypt** on Azure Local;
  export only tokenised or derived data to `${country.azure.primary_region}`.
- Apply an initiative named **`CBE Hybrid Landing Zone`** that:
  - denies regulated data services in `${country.azure.primary_region}` unless the
...
- Route regulated telemetry to an **in-country SIEM** and non-regulated
  telemetry to a workspace in `${country.azure.primary_region}`.
...
- [ ] A workload tagged `cbe-tier=regulated` is **denied** if someone tries to
      place it directly in `${country.azure.primary_region}`.
- [ ] The token vault / re-identification service remains reachable only from the
      in-country Azure Local tier.
- [ ] Data exported to `${country.azure.primary_region}` appears as tokens,
      masked values, or derived aggregates only.
- [ ] A DR restore drill in `${country.azure.paired_region}` proves the backup is
...
- If a data scientist says they only need analytics in `${country.azure.primary_region}`,
...
- Which telemetry can safely go to `${country.azure.primary_region}`, and which
...
- **Derived-data sprawl:** once teams see `${country.azure.primary_region}` as
...
| Tokenised analytics events, derived fraud features, masked reporting extracts | `${country.azure.primary_region}` | Cross-border only after tokenisation / permit. |
| DR copies of tokenised backups | `${country.azure.paired_region}` | Operational resilience without exposing raw identity. |
