...
> Walkthrough for `${country.summit_edition}` / Challenge EG-03.
...
- If the field is permitted to leave Egypt under the PDPC file, place the cloud
  workload in `${country.azure.primary_region}` and document the permit,
...
Pin the public-cloud path to `${country.azure.primary_region}` and
...
Raw PDPL-restricted fields in Egypt
  ...
  -> ${country.azure.primary_region} analytics / app tier
```
...
pipeline and proving that the value stored in `${country.azure.primary_region}` is
only a token.
...
- policy assignment showing only `${country.azure.primary_region}` /
  `${country.azure.paired_region}` are allowed;
...
- a before/after example showing raw restricted fields in Egypt and tokenised
  values in `${country.azure.primary_region}` only.
