# Challenge 1 — Sovereign guardrails for ${country.name} with Azure Policy & RBAC

**[Home](../Readme.md)** — Next: [Challenge 2](challenge-02.md)

> **Country edition:** ${country.summit_edition}
...
> **Primary law:** ${country.regulatory.primary_law}

## The situation

You have just been appointed cloud platform lead for **${country.scenarios.public_sector_tenant}**.
...
1. Personal information of South African data subjects is processed **only**
   in `${country.azure.primary_region}` or `${country.azure.paired_region}`.
...
- Map each Azure control back to the relevant section of
  ${country.regulatory.primary_law} so a non-technical regulator can follow
  your evidence.
...
- **`${country.azure.primary_region}` capacity:** some SKUs (especially
  newer Confidential Compute and certain Cosmos DB capabilities) are not yet
  available in `${country.azure.primary_region}`. Plan exemptions per *SKU*,
  not per *region*, so the residency rule itself never gets weakened.
- **Paired region awareness:** geo-redundant storage in
  `${country.azure.primary_region}` replicates to
  `${country.azure.paired_region}` — both are inside South Africa. Make sure
...
- Add a custom policy that **requires** diagnostic settings on every new
  ...
  `${country.azure.primary_region}` (this sets up Challenge 3 nicely).
