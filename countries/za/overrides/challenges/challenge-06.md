# Challenge 6 — Sovereign hybrid for a ${country.name} municipality
...
- Deploy a VM on Azure Local through the Azure portal and explain when
  this is the right answer vs deploying to `${country.azure.primary_region}`.
...
- Where do you draw the line between "this workload belongs on Azure
  Local" and "this workload belongs in `${country.azure.primary_region}`"?
...
- **LocalBox quotas / SKUs:** LocalBox deploys nested virtualisation;
  ...
  VMs (`${country.azure.azure_local_instance_location}` works well as a
  jumpstart region — for production, plan a true ${country.name}-based
  hardware footprint).
...
- Build a one-page architecture diagram showing which workloads live in
  `${country.azure.primary_region}`, which live in
  `${country.azure.paired_region}` for DR, and which live on Azure Local
