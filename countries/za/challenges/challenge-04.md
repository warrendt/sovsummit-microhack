...
Department of Home Affairs (DHA) digital identity workload runs the monthly payroll for the
...
- [ ] A virtual network and subnet exist in
      `southafricanorth` with private endpoints for the
...
- **CC SKU quota in `southafricanorth`:** new
...
- **Pairing for DR:** `southafricawest` has fewer CC SKUs
  than `southafricanorth` — design your DR runbook
  accordingly.
