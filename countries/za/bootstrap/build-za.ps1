...
$countryRoot = Split-Path -Parent $scriptDir
$prepDir     = Join-Path $countryRoot 'subscription-prep'
$demoDir     = Join-Path $countryRoot 'demo-vms'

if ($InternalHelpersPath) {
    $helpersDir = $InternalHelpersPath
} elseif ($env:SOVSUMMIT_INTERNAL_HELPERS) {
    $helpersDir = $env:SOVSUMMIT_INTERNAL_HELPERS
} else {
    $helpersDir = Join-Path $HOME 'Repos/SovSummit-Internal/preparation-helpers'
}

if ($Coach) {
    if (-not (Test-Path $prepDir)) {
        throw "Expected subscription-prep scripts at $prepDir but did not find them. Run this from countries/za/bootstrap/."
    }
    ...
    if ($CreateUsers) {
        ...
        if (-not $TapExportPath) { $TapExportPath = Join-Path $countryRoot 'TemporaryAccessPasses.xlsx' }
