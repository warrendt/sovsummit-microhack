...
Department of Home Affairs (DHA) digital identity workload is exposing a new citizen-facing
...
- [ ] A Log Analytics workspace in `southafricanorth`
...
- **Log Analytics region:** if you allowed the platform team to put the
  ...
  Pin the workspace in `southafricanorth` first, then enable
  diagnostics.
