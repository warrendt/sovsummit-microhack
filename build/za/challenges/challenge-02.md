...
You have moved across to **A South African tier-1 retail bank issuing virtual cards**. The bank
...
- Keys are stored in an HSM the bank can prove is geographically located in
  South Africa and is destroyable on demand.
...
- Provision a `Premium` Key Vault with **purge
...
- [ ] A `Premium` Key Vault exists in
      `southafricanorth` with `purgeProtection = true`,
...
- [ ] At least one key in the vault is HSM-protected
      ...
      expiry).
- [ ] A Storage account in `southafricanorth` has
...
- **Premium Key Vault availability:** `Premium` Key
  Vault is available in `southafricanorth`, but Azure
