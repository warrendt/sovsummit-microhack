...
| `--deploy-localbox` | off | Deploys LocalBox (~4-6 h async) into `rg-localbox` (`swedencentral` shell + Azure Local in `westeurope`). `--azure-local-instance-location` must stay in the LocalBox ValidateSet. |
